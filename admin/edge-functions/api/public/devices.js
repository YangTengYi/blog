/**
 * GET /api/public/devices  设备公开只读 API（Edge Functions，Edge Runtime）
 *
 * 无鉴权：middleware.js 白名单已放行 /api/public/*。供博客前台运行时 fetch 与构建期快照拉取。
 * 返回嵌套结构 { devices: Record<类别, Device[]>, total }，与管理端存储形状一致。
 * 响应头：CORS（PUBLIC_ALLOWED_ORIGIN）+ Cache-Control 60s（同 public/friends.js）。
 *
...
export async function onRequest(context) {
	...
	const kv = getKV();
	if (!kv) {
		return json({ devices: {}, total: 0 }, 200, cors);
	}

	const obj = await kv.get(DATA_KEY, { type: "json" });
	const devices =
		obj && typeof obj === "object" && !Array.isArray(obj) ? obj : {};
	return json({ devices, total: countDevices(devices) }, 200, cors);
}
