/**
 * GET /api/public/friends  友链公开只读 API（Edge Functions，Edge Runtime）
 *
 * 无鉴权：middleware.js 白名单已放行 /api/public/*。供博客前台运行时 fetch。
 * 只返回 enabled 的友链，按 weight 倒序（与博客 getEnabledFriends 排序一致）。
 * 响应头：CORS（PUBLIC_ALLOWED_ORIGIN）+ Cache-Control 60s（同 public/announcements.js）。
 *
...
export async function onRequest(context) {
	...
	const kv = getKV();
	if (!kv) {
		return json({ friends: [], total: 0 }, 200, cors);
	}

	const arr = await kv.get(DATA_KEY, { type: "json" });
	const list = Array.isArray(arr) ? arr : [];
	// 只保留启用项，按 weight 倒序（与博客 src/config/friendsConfig.ts getEnabledFriends 一致）
	const items = list
		.filter((f) => f && f.enabled !== false)
		.sort((a, b) => (b.weight || 0) - (a.weight || 0));
	return json({ friends: items, total: items.length }, 200, cors);
}
