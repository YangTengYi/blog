...
const OUTPUT_DATA_FILE = join(SRC_DIR, "constants", "icons-data.json");
...
/**
 * 获取单个图标的 SVG 及 body 数据
 */
async function getIconDataWithBody(iconName) {
	...
	let svg = iconToHTML(body, renderData.attributes);
	...
	return { svg, body, width: iconSet.width, height: iconSet.height };
}
...
/**
 * 生成 icons-data.json 文件（Iconify 图标集格式，供 @iconify/svelte/offline 使用）
 */
function generateIconsDataFile(iconSetsData) {
	const result = {};
	for (const [prefix, data] of Object.entries(iconSetsData)) {
		result[prefix] = {
			prefix,
			icons: data.icons,
			width: data.width,
...
async function main() {
	...
	for (const file of files) {
		// 跳过图标文件本身
		if (file.endsWith("icons.ts") || file.endsWith("icons-data.json")) continue;
	...
	for (const iconName of allIcons) {
		const result = await getIconDataWithBody(iconName);
		if (result) {
			const [prefix, name] = iconName.split(":");
			iconsMap.set(iconName, result.svg);

			// 构建 icons-data.json 格式
			if (!iconSetsData[prefix]) {
				iconSetsData[prefix] = {
					icons: {},
					width: result.width,
			...
			iconSetsData[prefix].icons[name] = {
				body: result.body,
			};
	...
	// 生成 icons-data.json 文件
	const dataOutput = generateIconsDataFile(iconSetsData);
	writeFileSync(OUTPUT_DATA_FILE, dataOutput, "utf-8");

	console.log(`📝 已生成: ${OUTPUT_DATA_FILE}`);
	console.log(`📦 文件大小: ${(Buffer.byteLength(dataOutput, "utf-8") / 1024).toFixed(2)} KB\n`);
}
